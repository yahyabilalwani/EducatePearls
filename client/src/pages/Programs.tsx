      ...
      <section className="bg-gradient-to-br from-green-700 via-green-600 to-emerald-600 text-white py-24 relative overflow-hidden">
        ...
        <div className="container-max text-center relative z-10">
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold mb-6 space-y-2">
            <span>Our</span>
            <span className="block text-emerald-200">Programs</span>
          </h1>
            ...
            {programs.map((program, index) => (
              <div key={index} className={`grid grid-cols-1 lg:grid-cols-2 gap-16 items-center ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
                <div className={index % 2 === 1 ? 'lg:order-2' : 'lg:order-1'}>
                    ...
                    <ul className="space-y-4">
                      {program.details.map((detail, idx) => (
                        <li key={idx} className="flex items-start group hover:translate-x-2 transition-transform">
                          <CheckCircle className="h-6 w-6 text-green-600 mr-4 mt-1 flex-shrink-0 group-hover:scale-110 transition-transform" />
                  ...
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200/50 p-8 rounded-2xl">
                    <div className="flex items-center">
                      <div className="text-3xl font-bold text-green-600 mr-6">{program.impact}</div>
                ...
                <div className={index % 2 === 1 ? 'lg:order-1' : 'lg:order-2'}>
                  <div className="glass-card-premium p-10 rounded-3xl hover:scale-105 transition-all duration-300 border border-green-100/50">
